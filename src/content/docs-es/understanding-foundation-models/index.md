...
| Familia | Ejemplos | Atención | Mejor para |
| --- | --- | --- | --- |
| **Solo encoder** | BERT, RoBERTa | Bidireccional (LM enmascarado) | Clasificación, embeddings, NER |
| **Solo decoder** | GPT, Llama, Claude | Causal (izquierda→derecha) | Generación, chat, agentes |
| **Encoder–decoder** | T5, BART, Transformer original | Ambas | Traducción, resumen (seq2seq) |

La mayoría de **foundation models** de chat en producción son LM **autoregresivos decoder-only**. Los checkpoints **solo encoder** siguen importando para **embeddings de retrieval** y rerankers (cap. 6).
...
**Intuición numérica:** Con **~10²³ FLOPs** de pre-entrenamiento, Chinchilla sugiere del orden de **~70B parámetros** con **~1,4T tokens**—no un 70B entrenado solo con 300B tokens (infraentrenado) ni un 7B con 1,4T a ese cómputo. La ficha del modelo en API rara vez lo dice; pregúntate si fue **compute-optimal** para su tamaño.
...
| Etapa | Qué optimizas | Datos | Salida típica |
| --- | --- | --- | --- |
| **SFT** | Imitar demostraciones | Pares `(instrucción, respuesta)` | Formato, tono conversacional |
| **Ajuste por preferencias** | Ganar vs. perder | Preferencias humanas o de IA | Útil, inofensivo, on-brand |
| **RLHF** (clásico) | Score del modelo de recompensa | Preferencias + entrenamiento RM | Cambio de política vía PPO |
| **DPO** (común hoy) | Pérdida directa de preferencias | Pares elegido/rechazado | Objetivos similares, entrenamiento más simple |

**Nota de ingeniería:** el SFT enseña **qué decir**; las preferencias enseñan **qué prefieren los usuarios** entre respuestas válidas. SFT malo enseña hechos erróneos; preferencias malas enseñan **adulación** (cap. 10).
...
| Perilla | Efecto | Subir cuando | Bajar cuando |
| --- | --- | --- | --- |
| **Temperatura** | Distribución aguda vs plana | Creatividad, brainstorming | JSON, cumplimiento, reproducibilidad |
| **Top-p** | Truncar masa de la cola | Equilibrio diversidad | Control estable de tokens raros |
| **Top-k** | Limitar candidatos | Evitar tokens absurdos | Top-k bajo elimina opciones válidas |
| **Penalización frec./presencia** | Menos repetición | Prosa larga que se repite | Respuestas factuales cortas |
...
### Taxonomía de alucinación

| Tipo | Causa | Mitigación (arco del libro) |
| --- | --- | --- |
| **Ruido de muestreo** | Decodificación aleatoria | Menor temperatura; best-of-N + verificador |
| **Brecha de conocimiento** | Hecho no en pesos | **RAG** (cap. 6), modelo más fresco, finetune (cap. 7–8) |
| **Fallo de contexto** | Prompt largo, aguja perdida | Chunking, colocación, resumen (cap. 5–6) |
| **Sobreconfianza / bola de nieve** | El modelo insiste en premisa falsa | Autocrítica, guardrails, revisión humana (cap. 3–4, 10) |
